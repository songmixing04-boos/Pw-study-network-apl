...
import { Menu, Sparkles, Palette } from 'lucide-react';
      ...
      <header className="fixed top-0 left-0 right-0 z-40 bg-background/80 backdrop-blur-lg border-b border-white/5 shadow-[0_4px_30px_rgba(0,0,0,0.5)]">
        <div className="container mx-auto px-4 h-20 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-6">
            ...
            {/* Theme picker button (desktop) */}
            <button
              onClick={() => setThemeOpen(true)}
              className="flex items-center gap-1.5 px-4 py-2 text-sm font-semibold rounded-lg bg-primary/10 text-primary border border-primary/20 hover:bg-primary hover:text-primary-foreground transition-all shadow-[0_0_10px_rgba(212,160,23,0.1)]"
            >
              <Palette size={14} />
              Themes
            </button>
          ...
          {/* Hamburger (mobile) — opens theme menu */}
          <button
            className="md:hidden flex items-center gap-2 text-foreground hover:text-primary transition-colors"
            onClick={() => setThemeOpen(true)}
